...
\noindent Os exemplos a seguir utilizam MQTT e Ricart-Agrawala como ilustra\c{c}\~ao, mas a arquitetura \'e independente dessas escolhas.
...
\begin{tikzpicture}[
    ...
    \node[note] at (10,-3.8) {N\~ao quer $[2,3]$};
...
Quando dois carrinhos disputam a mesma c\'elula simultaneamente, o algoritmo de Ricart-Agrawala resolve o conflito: o agente com menor \textit{timestamp} de Lamport obt\'em prioridade. Em caso de empate, o menor identificador de processo vence. O agente preterido enfileira a requisi\c{c}\~ao e aguarda a libera\c{c}\~ao do recurso para tentar novamente. A Figura~\ref{fig:seq-conflito} ilustra esse cen\'ario.
...
\end{tikzpicture}
\caption{Diagrama de sequ\^encia --- resolu\c{c}\~ao de conflito via Ricart-Agrawala}
...
\begin{tikzpicture}[
    ...
    % Agents
    \node[agent] (ca) at (-5.5, -4) {Carrinho A\\{\scriptsize Navega\c{c}\~ao + Lamport + Locks}};
    \node[agent] (cb) at (0, -4) {Carrinho B\\{\scriptsize Navega\c{c}\~ao + Lamport + Locks}};
    \node[agent] (cc) at (5.5, -4) {Carrinho C\\{\scriptsize Navega\c{c}\~ao + Lamport + Locks}};
